...
#include <Adafruit_LEDBackpack.h> //HT16K33 4 digit 14-Segment Display
...
void setup() {
 ...
 // Serial.begin(115200);                   // for debug
  displayshow("1.02", 1000, &display);  //show version
...
void loop() {
  ...
  if (menulevel == 11) {  //set F-STOPs steps
    ...
    displayset(temp_str.c_str(), &display);
  ...
  if (menulevel == 31) {  // enlarger timer
    ...
    if (pedal.pressed())
      expo();
    if (countdown == 0) {
      temp_str = String(e_time, 1);
      displayset(temp_str.c_str(), &display);
    } else {
      temp_str = String(countdown, 1);
      displayset(temp_str.c_str(), &display);
    }
    if (encbut == 1) {
      menulevel = TOP;
  ...
  if (menulevel == 41) {  //develop timer
    ...
    if (encbut == 2)
      if (_10x) {
        _10x = false;
        displayshow("+1.0", 500, &display);
      } else {
        _10x = true;
        displayshow("+10.0", 500, &display);
      }
...
void strip_expo() {  //strip expo
  ...
  while (s <= strip && rotary.pushType(1000) != 1) {
    ...
    temp_str = "S.-" + String(s);
    displayshow(temp_str.c_str(), 250, &display);
    displayshow("S.-", 250, &display);
    if (pedal.pressed()) {
      if (countdown == 0)
        if (s == 1)
      ...
      while (!pedal.pressed() && (countdown != 0)) {
        temp_str = String(countdown, 1);
        displayshow(temp_str.c_str(), 100, &display);  // float with one decimal place, display 100ms
...
void displayshow(char* str, int _delay, Adafruit_AlphaNum4* alphanum) {
  display.clear();
  int j = 0;
  if (str[0] >= '0' && str[0] <= '9')  //align right
  {
    j = strlen(str) - 1;
    for (int i = 3; i >= 0; i--) {
      if ('.' == str[j])  //if the next character after this one is '.'
      {
        j--;
        alphanum->writeDigitAscii(i, str[j], true);  //write the '.' along with the character
                                                         //skip over the '.'
      } else {
        alphanum->writeDigitAscii(i, str[j]);  //write the character
      }
      j--;
      if (j < 0)
        break;
  ...
  for (int i = 0; i < 4; i++)  //for each digit
  {
    if (str[j] == 0)
      break;
    if ('.' == str[j + 1])  //if the next character after this one is '.'
    {
      alphanum->writeDigitAscii(i, str[j], true);  //write the '.' along with the character
      j++;                                         //skip over the '.'
    } else {
...
void displayset(char* str, Adafruit_AlphaNum4* alphanum) {
  display.clear();
  int j = 0;
  if (str[0] >= '0' && str[0] <= '9')  //align right
  {
  ...
  for (int i = 0; i < 4; i++)  //for each character in str
  {
      if (str[j] == 0)
        break;
      if ('.' == str[j + 1])  //if the next character after this one is '.'
